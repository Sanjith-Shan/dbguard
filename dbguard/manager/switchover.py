...
async def _switchover(ctl: SetController, to: str | None) -> Event:
    ...
    if to is None:
        ...
        extra = views[to].have - pv.gtid_executed
        if extra:
            raise SwitchoverError(f"{to} has {extra.count()} transactions the primary lacks")
        cands = [to]
    ...
    final = GtidSet.parse(resp.get("gtid_executed")) if resp and resp.get("gtid_executed") \
        else None
    ...
    if not ok:
        note = f"{to} did not catch up ({why}), promoted {old} back"
        try:
