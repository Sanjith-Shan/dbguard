...
It promises that every write a client got an OK for is in at least one replica's relay log, and that DBGuard promotes the replica with the largest retrieved set after it has applied its relay log, so no acknowledged write is lost across a failover. That is only claimed under `AFTER_SYNC`, one-hour timeout, `wait_for_replica_count=1`, and only as measured. In the lab it was 0 lost acknowledged writes over 30 kills. The async baseline also lost 0 in 30 kills on one host, which says the single-host lab hides the async window, not that async is safe. Under a host-death injection with a 2 ms replication delay (in-flight replication packets dropped at the SIGKILL) the baseline lost 1160 acknowledged writes in 10 kills and DBGuard lost 0 in 10 under the identical injection.
